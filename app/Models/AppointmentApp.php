...
class AppointmentApp extends Model
{
    ...
    public static function tomorrow_appoiments()
    {
        ...
        on a.Tratamiento_Nr = treatments.Atencion where  a.id in (SELECT max(id) FROM appointment_apps where Fecha = '".$tomorrow."' or Fecha ='".$monday."'  group by Tratamiento_Nr) and Estado in ('No Confirmado','Agenda Online','Lista de espera') order by Hora_inicio asc") );
        }
        elseif ($tomorrow->format('l') == 'Sunday'){
            $monday = $tomorrow->copy()->addDays(1);
            return DB::select( DB::raw("select a.id,a.Profesional , a.Tratamiento_Nr, Estado,Nombre_paciente,Apellidos_paciente,Celular,Hora_inicio,TotalAtencion+Avance as TotalAtencion,Mail,Fecha from appointment_apps as a join treatments
        on a.Tratamiento_Nr = treatments.Atencion where  a.id in (SELECT max(id) FROM appointment_apps where Fecha = '".$monday."' or Fecha ='".$monday."'  group by Tratamiento_Nr) and Estado in ('No Confirmado','Agenda Online','Lista de espera') order by Hora_inicio asc") );
        }

        return DB::select( DB::raw("select a.id,a.Profesional , a.Tratamiento_Nr, Estado,Nombre_paciente,Apellidos_paciente,Celular,Hora_inicio,TotalAtencion+Avance as TotalAtencion,Mail,Fecha from appointment_apps as a join treatments
        on a.Tratamiento_Nr = treatments.Atencion where  a.id in (SELECT max(id) FROM appointment_apps where Fecha = '".$tomorrow."'  group by Tratamiento_Nr) and Estado in ('No Confirmado','Agenda Online','Lista de espera') order by Hora_inicio asc") );
    }
    ...
    public static function form_appoiments($day){
        ...
        on a.Tratamiento_Nr = treatments.Atencion where  a.id in (SELECT max(id) FROM appointments where Fecha = '".$day."'  group by Tratamiento_Nr) and Estado in ('No Confirmado','Agenda Online','Lista de espera) order by Fecha asc,Hora_inicio asc") );
    }
